...
if ($VolumeOrDirectory) {
    ...
    $timewarriorMount = ""
    if ($TIMEWARRIOR_DIRECTORY) {
        $timewarriorMount = "--mount type=bind,src=$TIMEWARRIOR_DIRECTORY,target=/root/.local/share/timewarrior"
    }

    $githubCopilotMount = ""
    if ($GITHUB_COPILOT_DIRECTORY) {
        $githubCopilotMount = "--mount type=bind,src=$GITHUB_COPILOT_DIRECTORY,target=/root/.config/github-copilot"
    }

    $opencodeMount = ""
    if ($OPENCODE_DIRECTORY) {
        $opencodeMount = "--mount type=bind,src=$OPENCODE_DIRECTORY,target=/root/.local/share/opencode"
    }
